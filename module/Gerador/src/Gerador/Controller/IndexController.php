...
//        $dir = dirname(__DIR__);
        $dir = '/home/tonos907/public_html';
        $dir_tmpl = $dir . '/module/Gerador/src/Gerador/Template';
        $dir_app = $dir . '/module/' . $modulo . '/src/' . $modulo;
        $dir_view = $dir . '/module/' . $modulo . '/view/' . strtolower($modulo) . '/' . strtolower($name);
        $dir_model = $dir . '/module/' . $modulo . '/src/' . $modulo . '/Model';
        $dir_form = $dir . '/module/' . $modulo . '/src/' . $modulo . '/Form';
        $dir_controller = $dir . '/module/' . $modulo . '/src/' . $modulo . '/Controller';
        $dir_factory = $dir . '/module/' . $modulo . '/src/' . $modulo . '/Controller/Factory';
...
// CRIA O DIRETÓRIO DOS MODEL
        if (!file_exists($dir_model)) {
            mkdir($dir_model);
        }
// CRIA O DIRETÓRIO DOS FORM
        if (!file_exists($dir_form)) {
            mkdir($dir_form);
        }
// CRIA O DIRETÓRIO DOS CONTROLLER
        if (!file_exists($dir_controller)) {
            mkdir($dir_controller);
        }
// CRIA O DIRETÓRIO DOS FACTORY
        if (!file_exists($dir_factory)) {
            mkdir($dir_factory);
        }
...
// LENDO ARQUIVOS
        ...
        $tmpl_controller_factory = $lib->rFile($dir_tmpl . '/ControllerFactory.php');
...
# Verifica se o factory já foi criado
        if (!$lib->existsFile($dir_app . '/Controller/Factory/' . $name . 'ControllerFactory.php')) {
            $tmpl_controller = $lib->replaceData('CtrlName', $name, $tmpl_controller_factory);
            $tmpl_controller = $lib->replaceData('CtrlNames', $plural, $tmpl_controller_factory);
            $lib->wFile($dir_app . '/Controller/Factory/' . $name . 'ControllerFactory.php', $tmpl_controller_factory);
            $_SESSION['success'][] = 'Controller Factory: ' . $dir_app . '/Controller/' . $name . 'ControllerFactory.php';
        } else {
            $_SESSION['error'][] = 'O Controller Factory já está criado e não pode ser substituido';
        }
